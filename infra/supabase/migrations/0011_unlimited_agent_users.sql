create table if not exists public.agent_unlimited_users (
  user_id uuid primary key references auth.users(id) on delete cascade,
  granted_at timestamptz not null default now()
);

alter table public.agent_unlimited_users enable row level security;
grant all privileges on public.agent_unlimited_users to service_role;

insert into public.agent_unlimited_users(user_id)
values ('63fe5724-6973-4b03-861b-5ac4580a780d')
...
begin
  ...
  select exists(
    select 1 from public.agent_unlimited_users where user_id = caller
  ) into unlimited;
  ...
  if not unlimited and coalesce(current_user_tokens, 0) >= 100000 then
  ...
  if not unlimited and coalesce(current_global_tokens, 0) >= 500000 then
  ...
  if unlimited then
    return query select -1::bigint, -1::bigint;
  else
