...
type SourceDocument = { id: string; source: string; title: string | null; content: string; source_url: string | null; published_at: string | null; fetched_at: string; metadata?: Record<string, unknown> }
...
export function PlayerProfile({ playerId }: { playerId: string }) {
  ...
  const summary = sources.data?.find(document => document.source === 'openai_summary')
  const rawSources = (sources.data || []).filter(document => document.source !== 'openai_summary')
  const bySource = rawSources.reduce<Record<string, SourceDocument[]>>((groups, document) => {
  ...
  return <div className="mt-6 space-y-5">
    ...
    <div className="grid gap-5 lg:grid-cols-[minmax(0,1fr)_320px]">
      <section className="panel overflow-hidden rounded-2xl">
        ...
        {summary && <div className="border-b border-white/[.06] bg-[#b7f34a]/[.035] p-5 sm:p-6"><div className="flex flex-wrap items-center justify-between gap-2"><h3 className="font-semibold text-[#d8faa0]">Cross-source brief</h3><span className="rounded-full border border-[#b7f34a]/15 bg-[#b7f34a]/[.06] px-2.5 py-1 text-[10px] text-[#a9c875]">AI-generated · verify below</span></div><p className="mt-4 whitespace-pre-wrap text-sm leading-7 text-[#b8c1bc]">{summary.content}</p><p className="mt-3 text-[10px] text-[#65716b]">Generated {date(summary.fetched_at)} from the stored source documents shown below.</p></div>}
        <div className="divide-y divide-white/[.06]">{Object.entries(bySource).map(([source, documents]) => <div key={source} className="p-5 sm:p-6"><div className="mb-3 flex items-center justify-between"><h3 className="font-semibold text-white">{sourceNames[source] || source}</h3><span className="rounded-full bg-white/[.05] px-2.5 py-1 text-[10px] text-[#78847e]">{documents.length} {documents.length === 1 ? 'item' : 'items'}</span></div><div className="space-y-3">{documents.slice(0, 5).map(document => <details key={document.id} className="group rounded-xl border border-white/[.07] bg-black/15 p-4"><summary className="cursor-pointer list-none text-sm font-medium text-[#c6ceca]"><span className="flex items-start justify-between gap-4"><span>{document.title || document.content.slice(0, 120).replace(/\s+/g, ' ')}</span><span className="text-[#65716b] transition group-open:rotate-90">→</span></span><span className="mt-2 block text-[10px] font-normal text-[#58635d]">Published {date(document.published_at)} · fetched {date(document.fetched_at)}</span></summary><p className="mt-4 whitespace-pre-wrap border-t border-white/[.06] pt-4 text-sm leading-6 text-[#9da7a2]">{document.content}</p>{document.source_url && <a href={document.source_url} target="_blank" rel="noreferrer" className="focus-ring mt-4 inline-flex rounded text-xs font-medium text-[#b7f34a] hover:underline">Open original ↗</a>}</details>)}</div></div>)}{!rawSources.length && <div className="px-6 py-14 text-center text-sm text-[#78847e]">No source documents have been collected for this player yet.</div>}</div>
      </section>
