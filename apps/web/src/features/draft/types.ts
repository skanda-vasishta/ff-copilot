...
export type DraftPlayer = { id: string; espn_id?: string | null; sleeper_id?: string | null; name: string; position: string | null; nfl_team: string | null; projected_total_points: number | null; median_rank: number | null; average_rank: number | null; injury_status: string | null }
...
export type DraftSession = { id: string; user_id: string; league_id: string; selected_team_id: string; season: number; name: string; source: 'manual' | 'espn_live' | 'sleeper_live'; external_league_id: string | null; external_team_id: string | null; status: 'setup' | 'active' | 'completed'; draft_type: 'snake' | 'linear'; team_order: string[]; round_count: number; current_overall_pick: number; revision: number; created_at: string; updated_at: string; completed_at: string | null }
