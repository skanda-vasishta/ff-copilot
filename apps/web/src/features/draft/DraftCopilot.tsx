...
export function DraftCopilot({ thread, season }: { thread: AgentThread | null; season: number }) {
  ...
  return <div className="flex min-h-0 flex-1 flex-col">
    <div className="flex min-h-12 items-center gap-2 border-b border-white/[.06] px-4 py-2"><div className="min-w-0 flex-1"><p className="text-[10px] font-semibold uppercase tracking-[.12em] text-[#c36761]">Draft Copilot</p><p className="mt-0.5 truncate text-[10px] text-[#7b7b7b]">Latest board included with every request</p></div>{models.data?.models.length&&modelSelection?<><select aria-label="Model" value={modelSelection.model} onChange={(event)=>chooseModel(event.target.value)} disabled={agent.status!=='idle'||savingModel} className="h-7 max-w-36 rounded-[5px] border-0 bg-white/[.035] px-2 text-[10px] text-[#a4a4a4] outline-none hover:bg-white/[.055] disabled:opacity-35">{models.data.models.map((model)=><option key={model.id} value={model.id}>{model.label}</option>)}</select><select aria-label="Reasoning effort" value={modelSelection.reasoningEffort} onChange={(event)=>chooseReasoning(event.target.value)} disabled={agent.status!=='idle'||savingModel} className="h-7 max-w-24 rounded-[5px] border-0 bg-white/[.035] px-2 text-[10px] capitalize text-[#a4a4a4] outline-none hover:bg-white/[.055] disabled:opacity-35">{models.data.models.find((model)=>model.id===modelSelection.model)?.efforts.map((effort)=><option key={effort} value={effort}>{effort}</option>)}</select></>:null}</div>
    <div ref={messagesRef} className="min-h-0 flex-1 space-y-5 overflow-y-auto px-4 py-5">
      {!agent.messages.length && <p className="text-xs leading-5 text-[#7d7d7d]">Ask who to take, compare candidates, or analyze what other teams need.</p>}
      {agent.messages.map((message) => <AgentMessage key={message.id} message={message} />)}
      {agent.status !== 'idle' && agent.status !== 'error' && <p className="text-xs text-[#7d7d7d]">{agent.status === 'running-tool' ? 'Checking player data…' : 'Thinking…'}</p>}
    ...
    <form onSubmit={submit} className="border-t border-white/[.06] p-3"><div className="flex items-end gap-2 rounded-[8px] border border-white/[.08] bg-white/[.025] p-1.5"><textarea rows={1} value={input} onChange={(event) => setInput(event.target.value)} onKeyDown={(event) => { if (event.key === 'Enter' && !event.shiftKey) { event.preventDefault(); event.currentTarget.form?.requestSubmit() } }} placeholder="Ask about this draft…" className="min-h-9 flex-1 resize-none bg-transparent px-2 py-2 text-xs text-[#efefef] outline-none"/><button disabled={!input.trim() || agent.status !== 'idle'} className="grid size-8 place-items-center rounded-[6px] bg-[#c94f49] text-[#ffffff] disabled:opacity-25">↑</button></div></form>
  </div>
}
