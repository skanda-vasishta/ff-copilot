...
export function DraftRoom() {
  ...
  if (scopeLoading) return <p className="p-8 text-sm text-[#858585]">Loading…</p>
  if (!scope) return <div className="mx-auto max-w-lg p-16 text-center"><h1 className="text-2xl font-semibold text-white">Select a team first</h1><p className="mt-2 text-sm text-[#858585]">Drafts use the active league and team.</p></div>
  ...
  return <div className="flex h-[calc(100dvh-3.5rem)] min-h-0 flex-col">
    <div className="flex h-12 shrink-0 items-center gap-2 border-b border-white/[.06] px-4">
      <button onClick={() => setSessionId(null)} className="h-8 rounded-[6px] px-2.5 text-xs text-[#8d8d8d] hover:bg-white/[.04] hover:text-white">← All drafts</button>
      <span className="h-4 w-px bg-white/[.07]"/><span className="truncate text-xs font-medium text-[#d2d2d2]">{state.data?.session.name}</span>
      {state.data && <><span className="ml-auto text-[10px] uppercase tracking-[.1em] text-[#767676]">{state.data.session.draft_type} · {state.data.session.round_count} rounds</span><span className="rounded-[5px] bg-[#c94f49]/10 px-2 py-1 text-[10px] text-[#e18a84]">{state.data.session.status}</span></>}
    </div>
    {state.isLoading ? <p className="p-8 text-sm text-[#858585]">Loading draft…</p> : state.data ? <ActiveDraft state={state.data} leagueExternalId={scope.team.league.external_id} onChanged={() => client.invalidateQueries({ queryKey: ['draft-state', sessionId] })} /> : null}
  </div>
}
...
function DraftLanding({ sessions, loading, leagueName, season, onOpen, onCreate, onRenamed }: { sessions: DraftSession[]; loading: boolean; leagueName: string; season: number; onOpen: (id: string) => void; onCreate: () => void; onRenamed: () => void }) {
  ...
  return <main className="mx-auto w-full max-w-5xl px-5 py-10 sm:py-14">
    <header className="flex flex-col justify-between gap-5 border-b border-white/[.07] pb-7 sm:flex-row sm:items-end"><div><p className="text-[10px] font-semibold uppercase tracking-[.14em] text-[#c36761]">{leagueName} · {season}</p><h1 className="mt-2 text-3xl font-semibold tracking-[-.035em] text-white">Drafts</h1><p className="mt-2 text-sm text-[#7d7d7d]">Resume a draft or start a new manual run.</p></div><div className="flex items-center gap-2"><a href="/draft/espn-bridge" className="inline-flex h-9 items-center rounded-[6px] border border-white/[.09] px-3 text-xs text-[#b6b6b6] hover:bg-white/[.03] hover:text-white">ESPN live setup</a><button onClick={onCreate} className="h-9 rounded-[6px] bg-[#c94f49] px-4 text-xs font-semibold text-[#ffffff]">New draft</button></div></header>
    {renameError&&<p className="mt-4 text-xs text-red-300">{renameError}</p>}
    {loading?<div className="mt-6 grid gap-3 sm:grid-cols-2">{[0,1,2,3].map((item)=><div key={item} className="h-36 animate-pulse rounded-[8px] border border-white/[.05] bg-white/[.02]"/>)}</div>:sessions.length?<div className="mt-6 grid gap-3 sm:grid-cols-2">{sessions.map((session)=>{const total=session.team_order.length*session.round_count,completed=Math.min(session.current_overall_pick-1,total),progress=total?Math.round(completed/total*100):0,isRenaming=renamingId===session.id;return <div key={session.id} onClick={()=>!isRenaming&&onOpen(session.id)} className="group cursor-pointer rounded-[8px] border border-white/[.07] bg-white/[.018] p-5 text-left transition hover:border-white/[.13] hover:bg-white/[.028]"><div className="flex items-start justify-between gap-4"><div className="min-w-0 flex-1">{isRenaming?<div className="flex gap-2" onClick={(event)=>event.stopPropagation()}><input autoFocus value={renameValue} onChange={(event)=>setRenameValue(event.target.value)} onKeyDown={(event)=>{if(event.key==='Enter')saveRename(session);if(event.key==='Escape')setRenamingId(null)}} className="h-8 min-w-0 flex-1 rounded-[5px] border border-white/[.1] bg-black/20 px-2 text-xs text-white"/><button onClick={()=>saveRename(session)} className="rounded-[5px] bg-[#c94f49] px-2 text-[10px] font-semibold text-[#ffffff]">Save</button><button onClick={()=>setRenamingId(null)} className="px-1 text-[10px] text-[#7b7b7b]">Cancel</button></div>:<div className="flex items-center gap-2"><h2 className="truncate text-base font-semibold text-[#e3e3e3] group-hover:text-white">{session.name}</h2><button aria-label={`Rename ${session.name}`} onClick={(event)=>{event.stopPropagation();setRenameError('');setRenamingId(session.id);setRenameValue(session.name)}} className="rounded-[4px] px-1.5 py-1 text-[10px] text-[#767676] opacity-0 hover:bg-white/[.05] hover:text-white group-hover:opacity-100">Rename</button></div>}<p className="mt-1 text-[10px] uppercase tracking-[.1em] text-[#767676]">{session.draft_type} · {session.team_order.length} teams · {session.round_count} rounds</p></div><span className={`rounded-[5px] px-2 py-1 text-[9px] font-medium uppercase tracking-[.08em] ${session.status==='completed'?'bg-white/[.05] text-[#909090]':'bg-[#c94f49]/10 text-[#e18a84]'}`}>{session.status}</span></div><div className="mt-7"><div className="flex justify-between text-[10px] text-[#767676]"><span>{completed} of {total} picks</span><span>{progress}%</span></div><div className="mt-2 h-1 overflow-hidden rounded-full bg-white/[.05]"><div className="h-full bg-[#c65a54]" style={{width:`${progress}%`}}/></div></div><div className="mt-4 flex items-center justify-between text-[10px] text-[#616161]"><span>Updated {new Date(session.updated_at).toLocaleDateString(undefined,{month:'short',day:'numeric',year:'numeric'})}</span><span className="text-[#949494] group-hover:text-[#e18a84]">Open →</span></div></div>})}</div>:<div className="mt-16 text-center"><p className="text-sm font-medium text-[#d9d9d9]">No draft runs yet</p><p className="mt-1 text-xs text-[#767676]">Create one to set the order and begin entering picks.</p><button onClick={onCreate} className="mt-5 h-9 rounded-[6px] border border-white/[.09] px-4 text-xs text-[#bdc4b9] hover:bg-white/[.03]">Create your first draft</button></div>}
  </main>
}
...
function DraftSetup({ scope, onCreated, onCancel }: { scope: NonNullable<ReturnType<typeof useActiveScope>['scope']>; onCreated: (id: string) => void; onCancel?: () => void }) {
  ...
  return <div className="mx-auto w-full max-w-[760px] px-5 py-10"><div className="flex items-start justify-between"><div><p className="text-[10px] uppercase tracking-[.14em] text-[#c36761]">Manual draft</p><h1 className="mt-2 text-2xl font-semibold text-white">Create draft workspace</h1></div>{onCancel && <button onClick={onCancel} className="text-xs text-[#858585]">Cancel</button>}</div>
    <div className="mt-6 flex border-b border-white/[.07]"><button onClick={() => setMode('manual')} className="border-b border-[#c94f49] px-3 py-2 text-xs text-white">Manual</button><button onClick={() => setMode('espn_live')} className="border-b border-transparent px-3 py-2 text-xs text-[#858585] hover:text-white">ESPN Live</button>{scope.team.league.provider==='sleeper'&&<button onClick={() => setMode('sleeper_live')} className="border-b border-transparent px-3 py-2 text-xs text-[#858585] hover:text-white">Sleeper Live</button>}</div>
    <form onSubmit={submit} className="mt-7 grid gap-5 rounded-[10px] border border-white/[.07] bg-white/[.02] p-5">
      <div className="grid gap-4 sm:grid-cols-3"><label className="text-[10px] uppercase tracking-[.1em] text-[#737373]">Name<input value={name} onChange={(e) => setName(e.target.value)} className="mt-1.5 h-9 w-full rounded-[6px] border border-white/[.08] bg-black/15 px-3 text-xs normal-case tracking-normal text-white"/></label><label className="text-[10px] uppercase tracking-[.1em] text-[#737373]">Type<select value={draftType} onChange={(e) => setDraftType(e.target.value as 'snake' | 'linear')} className="mt-1.5 h-9 w-full rounded-[6px] border border-white/[.08] bg-black/15 px-3 text-xs normal-case tracking-normal text-white"><option value="snake">Snake</option><option value="linear">Linear</option></select></label><label className="text-[10px] uppercase tracking-[.1em] text-[#737373]">Rounds<input type="number" min={1} max={40} value={roundCount} onChange={(e) => setRoundCount(Number(e.target.value))} className="mt-1.5 h-9 w-full rounded-[6px] border border-white/[.08] bg-black/15 px-3 text-xs normal-case tracking-normal text-white"/></label></div>
      <div><div className="flex items-center justify-between"><p className="text-[10px] uppercase tracking-[.1em] text-[#737373]">Draft order</p><p className="text-[10px] text-[#737373]">Imported from ESPN when available</p></div><div className="mt-2 divide-y divide-white/[.055] rounded-[7px] border border-white/[.07]">{order.map((id, index) => <div key={id} className="flex h-9 items-center gap-3 px-3 text-xs"><span className="w-5 font-mono text-[10px] text-[#737373]">{index + 1}</span><span className="flex-1 text-[#d0d0d0]">{byId.get(id)?.name || id}{id === scope.team.id ? ' (your team)' : ''}</span><button type="button" onClick={() => move(index,-1)} className="text-[#737373] hover:text-white">↑</button><button type="button" onClick={() => move(index,1)} className="text-[#737373] hover:text-white">↓</button></div>)}</div></div>
      {error && <p className="text-xs text-red-300">{error}</p>}<button disabled={order.length < 2} className="h-9 rounded-[6px] bg-[#c94f49] text-xs font-semibold text-[#ffffff] disabled:opacity-30">Create draft</button>
    </form>
  </div>
}
...
function SleeperDraftSetup({ scope, onCreated, onManual, onCancel }: { scope: NonNullable<ReturnType<typeof useActiveScope>['scope']>; onCreated: (id: string) => void; onManual: () => void; onCancel?: () => void }) {
  ...
  return <div className="mx-auto w-full max-w-[760px] px-5 py-10"><div className="flex items-start justify-between"><div><p className="text-[10px] uppercase tracking-[.14em] text-[#c36761]">Sleeper Live</p><h1 className="mt-2 text-2xl font-semibold text-white">Connect the league draft</h1></div>{onCancel&&<button onClick={onCancel} className="text-xs text-[#858585]">Cancel</button>}</div><div className="mt-6 flex border-b border-white/[.07]"><button onClick={onManual} className="border-b border-transparent px-3 py-2 text-xs text-[#858585]">Manual</button><button className="border-b border-[#c94f49] px-3 py-2 text-xs text-white">Sleeper Live</button></div><div className="mt-7 rounded-[10px] border border-white/[.07] bg-white/[.02] p-5"><label className="text-[10px] uppercase tracking-[.1em] text-[#737373]">Draft name<input value={name} onChange={(event)=>setName(event.target.value)} className="mt-2 h-9 w-full rounded-[6px] border border-white/[.08] bg-black/15 px-3 text-xs normal-case tracking-normal text-white"/></label><p className="mt-4 text-xs text-[#858585]">Draft order and picks will be read directly from Sleeper. No browser extension is required.</p><button disabled={loading||!teams.data?.length||!name.trim()} onClick={create} className="mt-5 h-9 w-full rounded-[6px] bg-[#c94f49] text-xs font-semibold text-[#ffffff] disabled:opacity-30">{loading?'Connecting…':'Create connected draft'}</button>{error&&<p className="mt-3 text-xs text-red-300">{error}</p>}</div></div>
}
...
function EspnDraftSetup({ scope, onCreated, onManual, onCancel }: { scope: NonNullable<ReturnType<typeof useActiveScope>['scope']>; onCreated: (id: string) => void; onManual: () => void; onCancel?: () => void }) {
  ...
  return <div className="mx-auto w-full max-w-[760px] px-5 py-10"><div className="flex items-start justify-between"><div><p className="text-[10px] uppercase tracking-[.14em] text-[#c36761]">ESPN Live</p><h1 className="mt-2 text-2xl font-semibold text-white">Connect an ESPN draft</h1></div>{onCancel&&<button onClick={onCancel} className="text-xs text-[#858585]">Cancel</button>}</div>
    <div className="mt-6 flex border-b border-white/[.07]"><button onClick={onManual} className="border-b border-transparent px-3 py-2 text-xs text-[#858585] hover:text-white">Manual</button><button className="border-b border-[#c94f49] px-3 py-2 text-xs text-white">ESPN Live</button></div>
    <div className="mt-7 rounded-[10px] border border-white/[.07] bg-white/[.02] p-5"><label className="text-[10px] uppercase tracking-[.1em] text-[#737373]">ESPN draft URL<input value={draftUrl} onChange={(event)=>setDraftUrl(event.target.value)} placeholder="https://fantasy.espn.com/football/draft?leagueId=…" className="mt-2 h-10 w-full rounded-[6px] border border-white/[.08] bg-black/15 px-3 text-xs normal-case tracking-normal text-white"/></label><button disabled={loading||!draftUrl.trim()} onClick={inspect} className="mt-3 h-9 rounded-[6px] border border-white/[.09] px-4 text-xs text-[#bdc4b9] disabled:opacity-30">{loading?'Connecting…':'Load draft'}</button></div>
    {preview&&<div className="mt-4 rounded-[10px] border border-white/[.07] bg-white/[.02] p-5"><div className="grid gap-4 sm:grid-cols-2"><label className="text-[10px] uppercase tracking-[.1em] text-[#737373]">Draft name<input value={name} onChange={(event)=>setName(event.target.value)} className="mt-2 h-9 w-full rounded-[6px] border border-white/[.08] bg-black/15 px-3 text-xs normal-case tracking-normal text-white"/></label><label className="text-[10px] uppercase tracking-[.1em] text-[#737373]">Your draft team<select value={teamId} onChange={(event)=>setTeamId(event.target.value)} className="mt-2 h-9 w-full rounded-[6px] border border-white/[.08] bg-black/15 px-3 text-xs normal-case tracking-normal text-white">{orderedTeams.map((team)=><option key={team!.id} value={team!.id}>{team!.name}</option>)}</select></label></div><div className="mt-5 divide-y divide-white/[.055] border-y border-white/[.06]">{orderedTeams.map((team,index)=><div key={team!.id} className="flex h-9 items-center gap-3 text-xs"><span className="w-6 font-mono text-[10px] text-[#737373]">{index+1}</span><span className="text-[#d0d0d0]">{team!.name}</span></div>)}</div><button disabled={loading||!name.trim()||!teamId} onClick={create} className="mt-5 h-9 w-full rounded-[6px] bg-[#c94f49] text-xs font-semibold text-[#ffffff] disabled:opacity-30">Create connected draft</button></div>}
...
function ActiveDraft({ state, leagueExternalId, onChanged }: { state: Awaited<ReturnType<typeof getDraftState>>; leagueExternalId: string; onChanged: () => void }) {
  ...
  return <div className="flex min-h-0 flex-1 flex-col">
    <section className="shrink-0 border-b border-white/[.06] bg-white/[.012]">
      <div className="flex items-center justify-between px-4 py-2"><div><p className="text-[9px] uppercase tracking-[.12em] text-[#737373]">On the clock</p><p className="mt-0.5 text-xs font-semibold text-white">Pick {session.current_overall_pick} · {currentTeam?.name || 'Complete'}</p></div><div className="flex items-center gap-3">{tickerAway&&<button onClick={goToLatestPick} className="rounded-[5px] bg-[#c94f49]/10 px-2 py-1 text-[9px] font-medium text-[#e18a84]">Latest pick →</button>}{session.source==='espn_live'&&<><button onClick={connectBridge} className={`rounded-[5px] border px-2 py-1 text-[9px] ${bridgeState?.connected?'border-[#c94f49]/25 text-[#e18a84]':'border-white/[.08] text-[#898989]'}`}>{bridgeState?.connected?`ESPN connected · ${bridgeState.picks.length} picks`:bridgeLeagueId?'Waiting for ESPN bridge':'Connect ESPN'}</button><button disabled={bridgeRefreshing||!bridgeLeagueId} onClick={refreshBridge} className="rounded-[5px] border border-white/[.08] px-2 py-1 text-[9px] text-[#9b9b9b] disabled:opacity-40">{bridgeRefreshing?'Refreshing…':'Refresh ESPN'}</button></>}<p className="text-[9px] text-[#737373]">Scroll to review · click × to remove</p></div></div>
      {bridgeError&&<p className="px-4 pb-2 text-[10px] text-red-300">{bridgeError}</p>}
      <div ref={tickerRef} onScroll={updateTickerPosition} className="flex gap-1.5 overflow-x-auto px-3 pb-3">{Array.from({length:total},(_,index)=>{const overall=index+1,teamId=teamForPick(session.team_order,session.draft_type,overall),pick=pickByOverall.get(overall),round=Math.floor(index/session.team_order.length)+1,isCurrent=overall===session.current_overall_pick;return <div ref={isCurrent?currentPickRef:undefined} key={overall} onClick={() => pick && setSelectedPlayerId(pick.player_id)} className={`group relative w-[132px] shrink-0 rounded-[6px] border px-2.5 py-2 ${pick?'cursor-pointer':''} ${isCurrent?'border-[#c94f49]/40 bg-[#c94f49]/10':'border-white/[.055] bg-white/[.018]'}`}><div className="flex items-center justify-between text-[8px] uppercase tracking-[.08em] text-[#737373]"><span>R{round} · {overall}</span>{pick&&<button aria-label={`Remove ${pick.player.name}`} onClick={(event)=>{event.stopPropagation();remove.mutate(overall)}} className="text-transparent group-hover:text-[#858585]">×</button>}</div><p className={`mt-1 truncate text-[10px] ${pick?'font-medium text-[#d8d8d8]':'text-[#858585]'}`}>{pick?.player.name||teamById.get(teamId)?.name}</p><p className="mt-0.5 truncate text-[8px] text-[#676767]">{teamById.get(teamId)?.name}</p></div>})}</div>
    </section>
    <div className="grid min-h-0 flex-1 lg:grid-cols-[minmax(520px,1fr)_minmax(460px,.85fr)]">
      ...
      <aside className="order-1 flex min-h-0 flex-col">
        <div className="flex h-11 shrink-0 items-end gap-4 border-b border-white/[.06] px-4"><button onClick={()=>setWorkspaceTab('players')} className={`h-full border-b px-1 text-[10px] font-semibold uppercase tracking-[.1em] ${workspaceTab==='players'?'border-[#c94f49] text-[#c94f49]':'border-transparent text-[#737373]'}`}>Players</button><button onClick={()=>setWorkspaceTab('rosters')} className={`h-full border-b px-1 text-[10px] font-semibold uppercase tracking-[.1em] ${workspaceTab==='rosters'?'border-[#c94f49] text-[#c94f49]':'border-transparent text-[#737373]'}`}>Rosters</button></div>
        {workspaceTab==='players'?<main className="flex min-h-0 flex-1 flex-col"><div className="shrink-0 border-b border-white/[.06] p-3"><div className="flex flex-wrap gap-2"><input value={search} onChange={(e)=>setSearch(e.target.value)} placeholder="Search available players" className="h-9 min-w-[180px] flex-1 rounded-[6px] border border-white/[.07] bg-white/[.025] px-3 text-xs text-white"/><div className="flex gap-1">{['ALL','QB','RB','WR','TE'].map((pos)=><button key={pos} onClick={()=>setPosition(pos)} className={`rounded-[5px] px-2 text-[10px] ${position===pos?'bg-[#c94f49] text-[#ffffff]':'text-[#858585] hover:bg-white/[.04]'}`}>{pos}</button>)}</div></div></div><div className="min-h-0 overflow-y-auto"><div className="grid grid-cols-[38px_minmax(0,1fr)_42px_64px_52px] border-b border-white/[.05] px-3 py-2 text-[9px] uppercase tracking-[.08em] text-[#737373]"><span>OVR</span><span>Player</span><span>Pos</span><span>Proj</span><span></span></div>{available.slice(0,300).map((player)=>{const rank=rankByPlayer.get(player.id);return <div key={player.id} onClick={()=>setSelectedPlayerId(player.id)} className="grid cursor-pointer grid-cols-[38px_minmax(0,1fr)_42px_64px_52px] items-center border-b border-white/[.045] px-3 py-2.5 text-xs hover:bg-white/[.035]"><span className="font-mono text-[10px] text-[#a7a7a7]">{rank?.overall?.toFixed(1)||'—'}</span><span className="truncate font-medium text-[#e3e3e3]">{player.name} <span className="text-[9px] font-normal text-[#737373]">{player.nfl_team}</span></span><span className="text-[10px] text-[#a7a7a7]">{player.position}{rank?.position?` ${rank.position.toFixed(1)}`:''}</span><span className="font-mono text-[10px] text-[#a7a7a7]">{player.projected_total_points?.toFixed(1)||'—'}</span><button disabled={session.status==='completed'||mutate.isPending} onClick={(event)=>{event.stopPropagation();mutate.mutate({playerId:player.id,overall:session.current_overall_pick})}} className="h-7 rounded-[5px] bg-[#c94f49]/10 text-[10px] font-semibold text-[#e18a84] hover:bg-[#c94f49]/15 disabled:opacity-30">Draft</button></div>})}</div></main>:<div className="flex min-h-0 flex-1 flex-col"><div className="border-b border-white/[.06] p-3"><select value={rosterTeamId} onChange={(event)=>setRosterTeamId(event.target.value)} className="h-9 w-full rounded-[6px] border border-white/[.08] bg-white/[.03] px-3 text-xs text-[#d2d2d2]">{displayTeams.map((team)=><option key={team.id} value={team.id}>{team.name}{session.source==='espn_live'?state.participants.find((participant)=>participant.id===team.id)?.is_user?' · Your team':'':team.id===session.selected_team_id?' · Your team':''}</option>)}</select></div><div className="min-h-0 overflow-y-auto">{rosterPicks.map((pick)=><button key={pick.id} onClick={()=>setSelectedPlayerId(pick.player_id)} className="flex w-full items-center gap-3 border-b border-white/[.045] px-4 py-3 text-left text-xs hover:bg-white/[.03]"><span className="w-7 font-mono text-[9px] text-[#737373]">{pick.overall_pick}</span><span className="flex-1 text-[#d0d0d0]">{pick.player.name}</span><span className="text-[10px] text-[#737373]">{pick.player.position}</span></button>)}{!rosterPicks.length&&<p className="p-5 text-xs text-[#737373]">No players drafted by this team yet.</p>}</div></div>}
      </aside>
    </div>
