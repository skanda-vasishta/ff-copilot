...
type Consensus = { items: Array<{ player: DraftPlayer; position_consensus_average: number }> }
...
function ActiveDraft({ state, onChanged }: { state: Awaited<ReturnType<typeof getDraftState>>; teams: DraftTeam[]; onChanged: () => void }) {
  ...
  const rankByPlayer = new Map(consensus.data?.map((row) => [row.player.id, row.position_consensus_average]))
  const available = (pool.data?.items || []).filter((player) => !drafted.has(player.id) && ['QB','RB','WR','TE'].includes(player.position || '')).filter((player) => position === 'ALL' || player.position === position).filter((player) => player.name.toLowerCase().includes(search.toLowerCase())).sort((a,b) => (rankByPlayer.get(a.id) ?? 999) - (rankByPlayer.get(b.id) ?? 999))
  ...
  return <div className="grid min-h-0 flex-1 lg:grid-cols-[260px_minmax(430px,1fr)_330px]">
    <aside className="min-h-0 overflow-y-auto border-r border-white/[.06] bg-white/[.015]"><div className="sticky top-0 border-b border-white/[.06] bg-[#0c0e0b]/95 px-3 py-3"><p className="text-[10px] uppercase tracking-[.12em] text-[#687063]">On the clock</p><p className="mt-1 text-sm font-semibold text-white">Pick {session.current_overall_pick} · {currentTeam?.name || 'Complete'}</p></div>{Array.from({length: session.round_count},(_,r) => <div key={r} className="border-b border-white/[.05]"><p className="px-3 py-2 text-[9px] font-semibold uppercase tracking-[.1em] text-[#687063]">Round {r+1}</p>{Array.from({length:session.team_order.length},(_,i) => { const overall=r*session.team_order.length+i+1, teamId=teamForPick(session.team_order,session.draft_type,overall), pick=pickByOverall.get(overall); return <div key={overall} className={`group flex min-h-8 items-center gap-2 px-3 py-1.5 text-[10px] ${overall===session.current_overall_pick?'bg-[#c9f958]/10':''}`}><span className="w-6 font-mono text-[#687063]">{overall}</span><span className="min-w-0 flex-1 truncate text-[#9fa79a]">{pick?.player.name || teamById.get(teamId)?.name}</span>{pick && <button onClick={() => remove.mutate(overall)} className="text-transparent group-hover:text-[#687063]">×</button>}</div>})}</div>)}</aside>
    <main className="flex min-h-0 flex-col border-r border-white/[.06]"><div className="shrink-0 border-b border-white/[.06] p-3"><div className="flex gap-2"><input value={search} onChange={(e)=>setSearch(e.target.value)} placeholder="Search available players" className="h-9 min-w-0 flex-1 rounded-[6px] border border-white/[.07] bg-white/[.025] px-3 text-xs text-white"/><div className="flex gap-1">{['ALL','QB','RB','WR','TE'].map((pos)=><button key={pos} onClick={()=>setPosition(pos)} className={`rounded-[5px] px-2 text-[10px] ${position===pos?'bg-[#c9f958] text-[#15200b]':'text-[#78847e] hover:bg-white/[.04]'}`}>{pos}</button>)}</div></div></div><div className="min-h-0 overflow-y-auto"><div className="grid grid-cols-[38px_minmax(0,1fr)_42px_70px_60px] border-b border-white/[.05] px-3 py-2 text-[9px] uppercase tracking-[.08em] text-[#687063]"><span>Rank</span><span>Player</span><span>Pos</span><span>Proj</span><span></span></div>{available.slice(0,300).map((player)=><div key={player.id} className="grid grid-cols-[38px_minmax(0,1fr)_42px_70px_60px] items-center border-b border-white/[.045] px-3 py-2.5 text-xs hover:bg-white/[.02]"><span className="font-mono text-[10px] text-[#687063]">{rankByPlayer.get(player.id)?.toFixed(1)||'—'}</span><span className="truncate font-medium text-[#dce1d6]">{player.name} <span className="text-[10px] font-normal text-[#687063]">{player.nfl_team}</span></span><span className="text-[#9fa79a]">{player.position}</span><span className="font-mono text-[10px] text-[#9fa79a]">{player.projected_total_points?.toFixed(1)||'—'}</span><button disabled={session.status==='completed'||mutate.isPending} onClick={()=>mutate.mutate({playerId:player.id,overall:session.current_overall_pick})} className="h-7 rounded-[5px] bg-[#c9f958]/10 text-[10px] font-semibold text-[#b8e65b] hover:bg-[#c9f958]/15 disabled:opacity-30">Draft</button></div>)}</div></main>
    <aside className="flex min-h-0 flex-col"><div className="max-h-[38%] overflow-y-auto border-b border-white/[.06]"><div className="sticky top-0 bg-[#0c0e0b]/95 px-4 py-3"><p className="text-[10px] font-semibold uppercase tracking-[.12em] text-[#8daa48]">Your roster</p><p className="mt-1 text-xs text-[#737b70]">{teamById.get(session.selected_team_id)?.name}</p></div>{myPicks.map((pick)=><div key={pick.id} className="flex items-center gap-2 border-t border-white/[.045] px-4 py-2.5 text-xs"><span className="w-7 font-mono text-[9px] text-[#687063]">{pick.overall_pick}</span><span className="flex-1 text-[#c8cec2]">{pick.player.name}</span><span className="text-[10px] text-[#687063]">{pick.player.position}</span></div>)}{!myPicks.length&&<p className="px-4 pb-4 text-xs text-[#687063]">No picks yet.</p>}</div><DraftCopilot thread={state.thread} season={session.season}/></aside>
  </div>
}
