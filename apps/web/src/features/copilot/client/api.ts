import type { AgentEvent, AgentThread, ModelStep } from "@ff-copilot/agent-runtime";
...
export type AgentModelOption = { id: string; label: string; description: string };

export async function getAgentModels() {
  const response = await fetch("/api/agent/models");
  const body = await response.json().catch(() => ({}));
  if (!response.ok) throw new Error(body.error || "Could not load models");
  return body as { models: AgentModelOption[] };
}
...
export async function setThreadModel(threadId: string, model: string) {
  ...
  return body as { thread: AgentThread };
}
