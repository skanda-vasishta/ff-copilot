...
export function AgentPanel() {
  ...
  return <div className="copilot-shell grid h-full min-h-0 overflow-hidden bg-[#080907] lg:grid-cols-[238px_minmax(0,1fr)]">
    ...
    <section className="flex min-h-0 min-w-0 flex-col">
      <header className="copilot-toolbar flex min-h-[58px] flex-wrap items-center justify-between gap-3 border-b border-white/[.055] bg-[#0a0b09]/60 px-4 py-2 backdrop-blur-xl sm:px-6">
        ...
        <div className="flex items-center gap-1">
          {models.data?.models.length && modelSelection ? <>
            <select aria-label="Model" value={modelSelection.model} onChange={(event) => chooseModel(event.target.value)} disabled={agent.status !== "idle" || savingModel} className="focus-ring h-7 rounded-[5px] border-0 bg-white/[.035] px-2 text-[10px] text-[#8e9689] outline-none transition hover:bg-white/[.055] hover:text-[#cbd1c5] disabled:opacity-35">{models.data.models.map((model) => <option key={model.id} value={model.id}>{model.label}</option>)}</select>
            <select aria-label="Reasoning effort" value={modelSelection.reasoningEffort} onChange={(event) => chooseReasoning(event.target.value)} disabled={agent.status !== "idle" || savingModel} className="focus-ring h-7 rounded-[5px] border-0 bg-white/[.035] px-2 text-[10px] capitalize text-[#8e9689] outline-none transition hover:bg-white/[.055] hover:text-[#cbd1c5] disabled:opacity-35">{models.data.models.find((model) => model.id === modelSelection.model)?.efforts.map((effort) => <option key={effort} value={effort}>{effort}</option>)}</select>
          </> : null}
          {thread && <button disabled={refreshingContext || agent.status !== "idle"} onClick={refreshContext} className="focus-ring h-7 rounded-[5px] px-2 text-[10px] text-[#70786c] transition hover:bg-white/[.035] hover:text-[#b7beb1] disabled:opacity-35">{refreshingContext ? "Refreshing…" : "↻ Refresh"}</button>}
        </div>
      </header>
      ...
      <form onSubmit={submit} className="copilot-composer-fade shrink-0 bg-gradient-to-t from-[#080907] via-[#080907]/95 to-transparent px-3 pb-3 pt-2 sm:px-8 sm:pb-5">
