...
export async function POST(request: Request) {
  ...
  try {
    if (continuing) {
      ...
      const context = `\n\n${formatThreadContext(contextSnapshot)}\n\nContext rules: All team names and rosters are already present. Treat roster ownership as authoritative: before proposing a trade, verify every outgoing player is on the stated sender and every incoming player is on a different team; never recommend acquiring a player the user already owns. Use player tools for deeper rankings, projections, news, and source documents. Zero records and points before games are played mean preseason, not missing context. Projection policy: use only projected fields explicitly labeled for the ${thread.team.league.season} season. Never present ${previousSeason} projections as current; ${previousSeason} data may be used only as completed historical ground truth. During preseason, ESPN position_rank is the ${previousSeason} positional finish—not a ${thread.team.league.season} draft, projection, or consensus rank. State that basis whenever using it. Older source documents may provide historical context but must not override ${thread.team.league.season} projections.`;
