  ...
  return <main className="mx-auto max-w-[1120px] px-4 py-7 sm:px-6 lg:px-8">
    ...
    {query.isLoading ? <Loading /> : query.isError ? <Empty title="Couldn’t load this matchup" body={query.error.message} /> : !matchup || !mine || !opponent ? <Empty title={`No matchup for Week ${data?.week || "—"}`} body="The schedule will appear after your league data is refreshed from ESPN." action="Manage league" href="/settings" /> : <>
      <section className="relative mt-6 overflow-hidden rounded-[18px] border border-white/[.08] bg-[#181a16]/70 px-5 py-6 sm:px-8 sm:py-8">
        ...
        <div className="flex flex-col items-center justify-center gap-3 sm:flex-row"><p className="font-mono text-[10px] uppercase tracking-[.15em] text-[#78847e]">Week {data.week} · {matchup.status === "final" ? "Final" : "Matchup preview"}</p><Link href={`/copilot?prompt=${encodeURIComponent(`Give me a concise Week ${data.week} matchup preview against ${opponent.name}. Compare both starting lineups, identify my biggest advantages and risks, flag injuries, and recommend the one move that would most improve my odds.`)}`} className="focus-ring rounded-[6px] border border-[#c9f958]/25 bg-[#c9f958]/10 px-3 py-1.5 text-[10px] font-semibold text-[#d6fb7a] transition hover:bg-[#c9f958]/15">✦ Preview with Copilot</Link></div>
