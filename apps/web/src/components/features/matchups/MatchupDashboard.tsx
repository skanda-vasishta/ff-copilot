...
export function MatchupDashboard() {
  ...
  const myProjection = myPlayers.length ? lineupProjection(myPlayers) : matchup ? matchup[`${mySide}_projected`] ?? 0 : 0;
  const opponentSide = mySide === "home" ? "away" : "home";
  const opponentProjection = opponentPlayers.length ? lineupProjection(opponentPlayers) : matchup ? matchup[`${opponentSide}_projected`] ?? 0 : 0;
  ...
  return <main className="mx-auto max-w-[1120px] px-4 py-7 sm:px-6 lg:px-8">
    <header className="flex flex-col justify-between gap-4 sm:flex-row sm:items-end">
      <div><p className="text-[10px] font-semibold uppercase tracking-[.12em] text-[#cf716b]">{scope.team.league.name || "Fantasy league"} · {scope.team.league.season}</p><h1 className="mt-1.5 text-[28px] font-semibold tracking-[-.035em] text-[#fafafa]">Weekly matchup</h1><p className="mt-1 text-xs text-[#858585]">Your head-to-head outlook and lineup edges.</p></div>
      {data?.available_weeks.length ? <label className="flex items-center gap-2 text-[10px] uppercase tracking-[.09em] text-[#787878]">Week<select value={data.week} onChange={(event) => setWeek(Number(event.target.value))} className="focus-ring h-9 rounded-[7px] border border-white/[.08] bg-[#242424] px-3 text-xs font-semibold text-[#fafafa]">{data.available_weeks.map((value) => <option key={value} value={value}>{value}</option>)}</select></label> : null}
    </header>
    ...
    {query.isLoading ? <Loading /> : query.isError ? <Empty title="Couldn’t load this matchup" body={query.error.message} /> : !matchup || !mine || !opponent ? <Empty title={`No matchup for Week ${data?.week || "—"}`} body="The schedule will appear after your league data is refreshed from ESPN." action="Manage league" href="/settings" /> : <>
      <section className="relative mt-6 overflow-hidden rounded-[18px] border border-white/[.08] bg-[#292929]/70 px-5 py-6 sm:px-8 sm:py-8">
        <div className="absolute inset-x-0 top-0 h-px bg-gradient-to-r from-transparent via-[#c94f49]/50 to-transparent" />
        <div className="flex flex-col items-center justify-center gap-3 sm:flex-row"><p className="font-mono text-[10px] uppercase tracking-[.15em] text-[#858585]">Week {data.week} · {matchup.status === "final" ? "Final" : "Matchup preview"}</p></div>
        <div className="mt-6 grid grid-cols-[1fr_auto_1fr] items-center gap-3 sm:gap-8">
          ...
          <div className="grid size-10 place-items-center rounded-full border border-white/[.08] bg-black/20 font-mono text-[9px] font-bold text-[#6d6d6d]">VS</div>
      ...
      <section className="mt-[18px] overflow-hidden rounded-[14px] border border-white/[.07] bg-[#222222]">
        <div className="border-b border-white/[.06] px-4 py-4 sm:px-5"><p className="text-[10px] font-semibold uppercase tracking-[.1em] text-[#787878]">Full box score</p><h2 className="mt-1 text-base font-semibold text-[#fafafa]">Starters and bench</h2></div>
        <div className="overflow-x-auto"><div className="min-w-[820px]">
          <ScoreHeader mine={mine.name} theirs={opponent.name} />
          <div className="grid grid-cols-2 border-b border-white/[.08] bg-white/[.025] text-[9px] font-semibold uppercase tracking-[.08em] text-[#737373]"><TableLabels /><TableLabels /></div>
          ...
          <div className="border-y border-white/[.07] bg-white/[.035] px-5 py-2 text-center text-[9px] font-semibold uppercase tracking-[.14em] text-[#838383]">Bench & reserve</div>
      ...
      <aside className="mt-[18px] grid gap-[18px] sm:grid-cols-2">
          ...
          <Card eyebrow="Game plan" title="This week"><p className="text-xs leading-5 text-[#929292]">{myProjection >= opponentProjection ? `You have a ${Math.abs(myProjection - opponentProjection).toFixed(1)}-point lineup edge. Protect it by checking injury statuses before kickoff.` : `You’re projected ${Math.abs(myProjection - opponentProjection).toFixed(1)} points behind. Look for upside at your weakest position.`}</p><Link href="/free-agents" className="focus-ring mt-4 inline-flex text-xs font-semibold text-[#c94f49]">Improve your lineup →</Link></Card>
      </aside>
    </>}
...
function weeklyProjection(player?: Player) { return Number(player?.projected_average_points ?? player?.weekly_projected_points ?? player?.average_points ?? 0); }
...
function TeamScore({ team, score, projection: projected, align, label }: { team: Team; score: number | null; projection: number; align: "left" | "right"; label: string }) { const hasScore = score != null; return <div className={align === "right" ? "text-right" : "text-left"}><p className="text-[9px] font-semibold uppercase tracking-[.12em] text-[#c36761]">{label}</p><h2 className="mt-1 truncate text-sm font-semibold text-[#fafafa] sm:text-lg">{team.name}</h2><p className="mt-3 font-mono text-3xl font-semibold tracking-[-.05em] text-white sm:text-5xl">{hasScore ? score.toFixed(1) : projected.toFixed(1)}</p><p className="mt-1 text-[10px] text-[#787878]">{hasScore ? `${projected.toFixed(1)} projected` : "projected points"}</p></div>; }
function WinBar({ mine, opponent }: { mine: number; opponent: number }) { const chance = mine + opponent ? Math.max(15, Math.min(85, 50 + (mine - opponent) * 1.5)) : 50; return <div className="mx-auto mt-7 max-w-xl"><div className="mb-2 flex justify-between text-[9px] uppercase tracking-[.1em] text-[#747474]"><span>Your edge</span><span>{Math.round(chance)}% outlook</span></div><div className="h-1.5 overflow-hidden rounded-full bg-white/[.06]"><div className="h-full rounded-full bg-gradient-to-r from-[#c65a54] to-[#df6a63]" style={{ width: `${chance}%` }} /></div></div>; }
function ScoreHeader({ mine, theirs }: { mine: string; theirs: string }) { return <div className="grid grid-cols-2 border-b border-white/[.08]"><div className="border-r border-white/[.08] px-5 py-3 text-sm font-semibold text-[#fafafa]">{mine} box score</div><div className="px-5 py-3 text-sm font-semibold text-[#fafafa]">{theirs} box score</div></div>; }
function TableLabels() { return <div className="grid grid-cols-[54px_minmax(0,1fr)_58px_58px] px-3 py-2"><span>Slot</span><span>Player, team · pos</span><span className="text-right">FF Proj</span><span className="text-right">FPTS</span></div>; }
...
function MatchupCopilot({ teamId, leagueId, season, week, opponent }: { teamId: string; leagueId: string; season: number; week: number; opponent: string }) {
  ...
  return <section className="mt-[18px] overflow-hidden rounded-[14px] border border-[#c94f49]/15 bg-[#c94f49]/[.025]">
    <div className="flex flex-wrap items-center justify-between gap-3 px-5 py-4"><div><p className="text-[10px] font-semibold uppercase tracking-[.1em] text-[#c36761]">Matchup Copilot</p><p className="mt-1 text-xs text-[#929292]">Get the matchup read without leaving this box score.</p></div><button type="button" onClick={open ? () => setOpen(false) : showPreview} className="focus-ring rounded-[6px] border border-[#c94f49]/25 bg-[#c94f49]/10 px-3 py-2 text-[10px] font-semibold text-[#f0aaa5] transition hover:bg-[#c94f49]/15">✦ {open ? "Hide preview" : "Preview with Copilot"}</button></div>
    {open && <div className="border-t border-white/[.06]">
      <div className="max-h-[520px] space-y-5 overflow-y-auto px-5 py-5 sm:px-7">
        {(creating || (thread && agent.status !== "idle" && !agent.messages.length)) && <p className="text-xs text-[#858585]">Building your matchup preview…</p>}
        {agent.messages.map((message) => <AgentMessage key={message.id} message={message} />)}
        {agent.status !== "idle" && agent.status !== "error" && agent.messages.length > 0 && <p className="text-xs text-[#858585]">{agent.status === "running-tool" ? "Checking league data…" : "Thinking…"}</p>}
        {agent.error && <p className="text-xs text-red-300">{agent.error}</p>}
      </div>
      {thread && <form onSubmit={submit} className="flex items-end gap-2 border-t border-white/[.06] p-3"><textarea aria-label="Ask a matchup follow-up" rows={1} value={input} onChange={(event) => setInput(event.target.value)} placeholder="Ask a follow-up about this matchup…" className="min-h-9 flex-1 resize-none rounded-[7px] border border-white/[.08] bg-white/[.025] px-3 py-2 text-xs text-[#efefef] outline-none focus:border-[#c94f49]/25"/><button disabled={!input.trim() || agent.status !== "idle"} className="h-9 rounded-[6px] bg-[#c94f49] px-3 text-[10px] font-semibold text-[#ffffff] disabled:opacity-25">Send</button><Link href="/copilot" className="focus-ring grid h-9 place-items-center rounded-[6px] px-2 text-[10px] text-[#858585] hover:text-[#e5e5e5]">Full chat →</Link></form>}
    </div>}
  </section>;
}
function Card({ eyebrow, title, children }: { eyebrow: string; title: string; children: React.ReactNode }) { return <section className="rounded-[14px] border border-white/[.06] bg-white/[.022] p-5"><p className="text-[10px] font-semibold uppercase tracking-[.1em] text-[#787878]">{eyebrow}</p><h2 className="mt-1 text-base font-semibold text-[#fafafa]">{title}</h2><div className="mt-4 space-y-3">{children}</div></section>; }
function Metric({ label, value, accent = false }: { label: string; value: string; accent?: boolean }) { return <div className="flex items-center justify-between border-t border-white/[.05] pt-3 text-xs"><span className="text-[#7b7b7b]">{label}</span><span className={`font-mono ${accent ? "text-[#c94f49]" : "text-[#e5e5e5]"}`}>{value}</span></div>; }
...
function Empty({ title, body, action, href }: { title: string; body: string; action?: string; href?: string }) { return <div className="mx-auto mt-16 max-w-lg rounded-[14px] border border-white/[.06] bg-white/[.022] px-6 py-16 text-center"><span className="font-mono text-xs text-[#c94f49]">VS</span><h2 className="mt-4 text-lg font-semibold text-[#fafafa]">{title}</h2><p className="mt-2 text-sm leading-6 text-[#858585]">{body}</p>{action && href ? <Link href={href} className="focus-ring mt-5 inline-flex rounded-[6px] bg-[#c94f49] px-4 py-2.5 text-xs font-semibold text-[#ffffff]">{action}</Link> : null}</div>; }
