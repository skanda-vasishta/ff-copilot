...
export function TeamDashboard() {
  ...
  return <div className="mx-auto max-w-[1120px] px-3 py-4 sm:px-6 sm:py-7 lg:px-8">
    <header className="rounded-[14px] border border-white/[.08] bg-[#181a16]/60 px-4 py-4 backdrop-blur-xl sm:rounded-[18px] sm:px-6"><div className="flex flex-col justify-between gap-4 sm:flex-row sm:items-end"><div className="min-w-0"><p className="truncate text-[10px] font-semibold uppercase tracking-[.1em] text-[#9dbe4e]">{scope.team.league.name || `${scope.team.league.provider} league`} · {scope.team.league.provider} · {scope.team.league.season}</p><h1 className="mt-1.5 truncate text-2xl font-semibold tracking-[-.03em] text-[#eef1e9] sm:text-[28px]">{scope.team.name}</h1></div><div className="grid grid-cols-3 gap-3 text-sm sm:flex sm:gap-6"><Stat label="Record" value={me ? `${me.wins || 0}-${me.losses || 0}${me.ties ? `-${me.ties}` : ""}` : "—"} /><Stat label="Standing" value={me?.standing ? `#${me.standing}` : "Preseason"} /><Stat label="Scoring" value={scope.team.league.scoring_format_label?.replace("_", " ") || "Custom"} /></div></div></header>
    ...
    <div className="mt-[18px] grid gap-[18px] lg:grid-cols-[minmax(0,1.6fr)_minmax(280px,1fr)]">
      <section className="overflow-hidden rounded-[14px] border border-white/[.06] bg-white/[.022]"><div className="flex flex-col gap-3 border-b border-white/[.06] px-4 py-3.5 sm:flex-row sm:items-end sm:justify-between sm:px-5"><div><p className="text-[10px] font-semibold uppercase tracking-[.1em] text-[#6e7568]">Roster</p><h2 className="mt-1 text-base font-semibold text-[#eef1e9]">{recommended ? "Recommended lineup" : "ESPN lineup"}</h2></div><div className="flex items-center justify-between gap-3 sm:justify-start">{roster.data?.snapshot && <p className="font-mono text-[10px] text-[#6e7568]">Updated {new Date(roster.data.snapshot.fetched_at).toLocaleDateString()}</p>}<button disabled={lineupBusy || !roster.data?.players.length || !slots.length} onClick={() => void lineupAgent.run("Set my best lineup for the ESPN starting slots. Return only the required structured JSON result.")} className="focus-ring h-9 rounded-[6px] bg-[#c9f958]/10 px-3 text-[10px] font-semibold text-[#c9f958] hover:bg-[#c9f958]/15 disabled:opacity-35 sm:h-8">{lineupBusy ? "Optimizing…" : recommended ? "Recalculate" : "Set best lineup"}</button></div></div>
...
function Stat({ label, value }: { label: string; value: string }) { return <div className="min-w-0"><p className="text-[9px] uppercase tracking-[.09em] text-[#6e7568]">{label}</p><p className="mt-1 truncate font-mono text-[13px] capitalize text-[#eef1e9]">{value}</p></div>; }
