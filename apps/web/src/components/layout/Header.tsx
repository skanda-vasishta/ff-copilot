  ...
  return <>
    <header className="sticky top-0 z-30 border-b border-white/[.07] bg-[#080b0e]/85 backdrop-blur-xl">
      ...
      <Link href="/team" className="focus-ring flex shrink-0 items-center gap-2.5 rounded-lg" aria-label="FF Copilot home">
        <span className="grid size-7 place-items-center rounded-md bg-[#b7f34a] text-[10px] font-black tracking-[-.05em] text-[#10140a]">FF</span>
      ...
      <nav className="ml-5 flex flex-1 items-center gap-1 sm:ml-10" aria-label="Primary navigation">
        {links.map(({ href, label, icon }) => {
          ...
          return <Link key={href} href={href} aria-current={active ? 'page' : undefined} className={`focus-ring flex h-14 items-center gap-1.5 border-b-2 px-2.5 text-[13px] font-medium transition ${active ? 'border-[#b7f34a] text-white' : 'border-transparent text-[#8c9992] hover:text-white'}`}>
            <span className="hidden text-base leading-none sm:inline" aria-hidden>{icon}</span><span>{label}</span>
          </Link>
        })}
      </nav>
      <button onClick={() => setSettingsOpen(true)} className="focus-ring flex max-w-52 items-center gap-2 rounded-md border border-white/[.09] px-2.5 py-1.5 text-left text-[11px] text-[#a8b2ad] transition hover:border-white/[.18] hover:bg-white/[.04] hover:text-white"><span className="hidden min-w-0 sm:block"><span className="block truncate font-medium">{scope?.team.name || 'Select team'}</span><span className="block truncate text-[9px] text-[#58635d]">{scope ? `${scope.team.league.name || 'League'} · ${scope.team.league.season}` : 'Workspace settings'}</span></span><span className="text-sm">⚙</span></button>
      </div>
    </header>
