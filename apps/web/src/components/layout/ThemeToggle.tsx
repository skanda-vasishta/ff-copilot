...
export function ThemeToggle() {
  ...
  return <button type="button" onClick={toggle} aria-label={`Use ${light ? 'dark' : 'light'} theme`} title={`Use ${light ? 'dark' : 'light'} theme`} className="theme-toggle focus-ring grid size-8 place-items-center rounded-[6px] text-[15px] transition"><span aria-hidden>{light ? '☾' : '☀'}</span></button>
}
