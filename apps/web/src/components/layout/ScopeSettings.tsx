...
export function ScopeSettings({ open, onClose }: { open: boolean; onClose: () => void }) {
  ...
  return <div className="fixed inset-0 z-50 flex justify-end bg-black/60 backdrop-blur-sm" role="dialog" aria-modal="true" aria-label="Workspace settings" onMouseDown={(event) => event.target === event.currentTarget && onClose()}>
    <aside className="h-full w-full max-w-md overflow-y-auto border-l border-white/[.08] bg-[#0c1013] p-5 shadow-2xl sm:p-7">
      ...
      <section className="panel mt-7 rounded-lg p-4">
        <label className="block text-[10px] font-semibold uppercase tracking-[.14em] text-[#65716b]">League</label>
        <select value={leagueId} onChange={(event) => setLeagueId(event.target.value)} className="focus-ring mt-2 w-full rounded-md border border-white/[.09] bg-[#090d10] px-3 py-3 text-sm text-white">
        ...
        <select aria-label="Workspace season" value={leagueId} onChange={(event) => setLeagueId(event.target.value)} className="focus-ring mt-2 w-full rounded-md border border-white/[.09] bg-[#090d10] px-3 py-3 text-sm text-white">
        ...
        <div className="mt-2 grid gap-2">{teams.data?.map((team) => {
          ...
          return <button key={team.id} onClick={async () => { await setTeam(team.id); onClose(); }} className={`focus-ring flex items-center justify-between rounded-md border px-3.5 py-3 text-left text-sm transition ${active ? "border-[#b7f34a]/35 bg-[#b7f34a]/10 text-[#d7fb9e]" : "border-white/[.08] text-[#aab4af] hover:bg-white/[.04] hover:text-white"}`}><span>{team.name}</span><span>{active ? "Selected" : "Select"}</span></button>;
        })}</div>
      ...
      <form onSubmit={submit} className="mt-6 border-t border-white/[.07] pt-6">
        ...
        <div className="mt-3 grid grid-cols-[1fr_100px] gap-2"><input required inputMode="numeric" value={externalId} onChange={(event) => setExternalId(event.target.value)} placeholder="ESPN league ID" className="focus-ring rounded-md border border-white/[.09] bg-[#090d10] px-3 py-3 text-sm text-white placeholder:text-[#4f5a54]" /><input aria-label="Season" required type="number" min="2025" max="2100" value={season} onChange={(event) => setSeason(Number(event.target.value))} className="focus-ring rounded-md border border-white/[.09] bg-[#090d10] px-3 py-3 text-sm text-white" /></div>
        {link.error && <p className="mt-2 text-xs text-red-300">{link.error.message}</p>}
        <button disabled={link.isPending} className="focus-ring mt-3 w-full rounded-md border border-[#b7f34a]/25 bg-[#b7f34a]/10 px-3 py-3 text-sm font-semibold text-[#c8f775] hover:bg-[#b7f34a]/15 disabled:opacity-50">{link.isPending ? "Connecting…" : "Connect ESPN league"}</button>
      </form>
      <button onClick={async () => { await createClient().auth.signOut(); location.assign("/login"); }} className="focus-ring mt-8 w-full rounded-md border border-white/[.08] px-3 py-3 text-sm text-[#8c9992] hover:border-red-300/20 hover:text-red-200">Sign out</button>
    </aside>
  </div>;
}
