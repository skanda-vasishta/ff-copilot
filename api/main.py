...
class LeagueMetrics:
    def __init__(self, league_id, year, team_name, team_id):
        ...
        self.bucket_name = 'ff-copilot-store'
        self.s3_client = self._initialize_s3_client()

        self.stats_df = self._load_csv_from_s3('player_stats.csv')
        self.sentiment_df = self._load_csv_from_s3('player_scraped_info.csv')
    ...
    def _load_csv_from_s3(self, key):
        try:
            obj = self.s3_client.get_object(Bucket=self.bucket_name, Key=key)
            data = obj['Body'].read().decode('utf-8')
            return pd.read_csv(StringIO(data))
        except ClientError as e:
            print(f"Failed to load {key} from s3://{self.bucket_name}: {e}")
            raise
        except Exception as e:
