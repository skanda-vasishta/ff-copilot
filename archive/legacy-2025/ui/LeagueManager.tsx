...
type LinkedLeague = { created_at: string; state: 'available' | 'being_prepared'; league: { id: string; name: string | null; external_id: string; season: number; status: string; last_synced_at: string | null } }
...
export function LeagueManager() {
  ...
  const link = useMutation({ mutationFn: () => api('/v1/me/leagues', { method: 'POST', body: JSON.stringify({ provider: 'espn', external_id: leagueId, season }) }), onSuccess: () => { setLeagueId(''); client.invalidateQueries({ queryKey: ['my-leagues'] }) } })
  ...
  return <div className="grid gap-5 lg:grid-cols-[minmax(0,1fr)_360px]">
    <section className="panel overflow-hidden rounded-2xl">
      ...
      <div className="divide-y divide-white/[.06]">{leagues.data?.map(({ league, state }) => { const available = state === 'available'; return <article key={league.id} className="p-5 sm:p-6"><div className="flex flex-col justify-between gap-3 sm:flex-row sm:items-start"><div className="flex gap-3"><span className="grid size-10 shrink-0 place-items-center rounded-xl bg-sky-300/[.08] text-sm font-bold text-sky-200">E</span><div><h3 className="font-medium text-white">{league.name || `ESPN League ${league.external_id}`}</h3><p className="mt-1 text-xs text-[#65716b]">ID {league.external_id} · latest season {league.season}</p></div></div><div className="sm:text-right"><span className={`inline-flex items-center gap-1.5 text-xs ${available ? 'text-[#b7f34a]' : 'text-sky-200'}`}><span className={`size-1.5 rounded-full bg-current ${available ? '' : 'animate-pulse'}`} />{available ? 'Available' : 'Being prepared'}</span><p className="mt-1 text-[10px] text-[#58635d]">{available && league.last_synced_at ? `Updated ${new Date(league.last_synced_at).toLocaleString()}` : available ? 'Ready to use' : 'We’ll make it available here soon'}</p></div></div>{available && <div className="mt-5 flex flex-wrap items-center gap-3 border-t border-white/[.06] pt-4"><Link href={`/leagues/${league.id}`} className="focus-ring rounded-lg bg-[#b7f34a] px-3 py-2 text-xs font-bold text-[#10140a] transition hover:bg-[#c7ff5e]">Open league dashboard →</Link><LeagueTeams leagueId={league.id} /></div>}</article> })}
    ...
    <aside className="space-y-5">
      <form onSubmit={submit} className="panel rounded-2xl p-5"><p className="text-[10px] font-semibold uppercase tracking-[.17em] text-[#b7f34a]">Add a league</p><h2 className="mt-2 font-semibold text-white">Connect public ESPN</h2><p className="mt-2 text-xs leading-5 text-[#78847e]">Enter a public league ID. If it&apos;s new to FF Copilot, we&apos;ll prepare it and show it here when it&apos;s ready.</p>
        ...
        {link.error && <p role="alert" className="mt-3 text-xs leading-5 text-red-300">{link.error.message}</p>}<button disabled={link.isPending} className="focus-ring mt-4 w-full rounded-xl bg-[#b7f34a] px-3 py-3 text-sm font-bold text-[#10140a] transition hover:bg-[#c7ff5e] disabled:opacity-50">{link.isPending ? 'Connecting…' : 'Connect league'}</button>
      </form>
      <section className="rounded-2xl border border-white/[.07] bg-white/[.025] p-5"><h2 className="text-sm font-semibold text-white">What happens next?</h2><p className="mt-2 text-xs leading-5 text-[#78847e]">Available leagues open immediately. New leagues may take a little time to prepare; you can leave this page and come back later.</p></section>
    </aside>
  </div>
}
